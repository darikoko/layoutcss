...
var layoutConfigSchema = import_zod.z.object({
  input: import_zod.z.object({
    directory: import_zod.z.string(),
    extensions: import_zod.z.array(import_zod.z.string())
  }),
  style: import_zod.z.object({
    harmonicRatio: import_zod.z.number(),
    baseValue: import_zod.z.string(),
    dev: import_zod.z.boolean()
  }),
  output: import_zod.z.object({
    file: import_zod.z.string(),
    minify: import_zod.z.boolean()
  })
});
...
async function loadLayoutConfigFromJson(path2 = "./layoutcss.json") {
  if ((0, import_fs.existsSync)(path2)) {
    try {
      const text = await (0, import_promises.readFile)(path2, "utf-8");
    ...
    try {
      await (0, import_promises.writeFile)(path2, JSON.stringify(defaultConfig, null, 2));
...
var resetCssPath = import_path.default.join(__dirname, "css", "reset.css");
var devCssPath = import_path.default.join(__dirname, "css", "dev.css");
var RESET_CSS = (0, import_fs2.readFileSync)(resetCssPath, { encoding: "utf8" });
var DEV_CSS = (0, import_fs2.readFileSync)(devCssPath, { encoding: "utf8" });
var componentMap = {
  "area-l": Area,
  "box-l": Box,
  "center-l": Center,
...
function cssProcess(path2, finalMap, config) {
  (0, import_fs3.readFile)(path2, "utf8", (err, data) => {
    if (err) {
      console.error(err);
      return;
    }
    ...
    (0, import_fs3.writeFile)(config.output.file, css, "utf8", (err2) => {
      if (err2) {
...
async function main() {
  ...
  const watcher = import_chokidar.default.watch(config.input.directory, {
    persistent: true,
    ignored: (path2) => {
      try {
        if ((0, import_fs3.statSync)(path2).isDirectory()) {
          return false;
        }
      } catch {
        return false;
  ...
  const configWatcher = import_chokidar.default.watch("./layoutcss.json", {
    persistent: true
  });
